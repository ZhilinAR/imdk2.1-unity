...
namespace Immersal
{
    public class ImmersalSDK : MonoBehaviour
    {
        ...
        private void AutoDetectPlatform()
        {
            try
            {
                // если уже задано руками — выходим
                if (m_Platform != null)
                    return;

                // 1) Пытаемся найти ARFoundationSupport в дочерних объектах
                ...
                // 2) Если активен OpenXR-лоадер, берём любую реализацию IPlatformSupport из детей
                if (IsOpenXRActive())
                {
                    IPlatformSupport anySupport = GetComponentsInChildren<MonoBehaviour>(true)
                        .OfType<IPlatformSupport>()
                        .FirstOrDefault();
                    if (anySupport != null)
                    {
                        m_Platform = anySupport as Object;
                        return;
                    }
                }
            }
        ...
        // Проверка активного OpenXR через reflection (без прямых зависимостей)
